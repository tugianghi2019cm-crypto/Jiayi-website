...
describe("TEST-DB-01 — accepts a syntactically valid PostgreSQL DATABASE_URL", () => {
  it.each([
    "postgresql://user:pass@localhost:5432/example",
    "postgres://user:pass@localhost:5432/example",
    "postgresql://user:pass@db.example.com:5432/example?sslmode=require",
  ])("accepts %s", (value) => {
...
describe("TEST-DB-03 — rejects an invalid DATABASE_URL", () => {
  it.each([
    "not-a-url",
    "mysql://user:pass@localhost:3306/example",
    "http://localhost:5432/example",
    "",
  ])("rejects %s", (value) => {
    process.env.DATABASE_URL = value;
    expect(() => getDatabaseConfig()).toThrow(/Database configuration invalid/);
...
describe("TEST-DB-04 — DATABASE_URL error redaction", () => {
  it("never echoes the configured DATABASE_URL value, valid or invalid", () => {
    const secretLookingUrl =
      "postgresql://ci_user:super-secret-db-password@db.internal.example.com:5432/jiayi";
    process.env.DATABASE_URL = secretLookingUrl;
    // This value is a valid postgresql:// URL, so getDatabaseConfig()
    // will not throw for it — the redaction guarantee instead matters
    // for the *invalid* case, tested below, and for TEST-08's general
    // redaction coverage. This case documents that a valid URL is
    // simply accepted, not inspected or logged anywhere.
    expect(() => getDatabaseConfig()).not.toThrow();

    process.env.DATABASE_URL = "mysql://ci_user:super-secret-db-password@localhost/jiayi";
    let thrown: unknown;
    try {
      getDatabaseConfig();
    } catch (error) {
    ...
    expect((thrown as Error).message).not.toContain("super-secret-db-password");
    expect((thrown as Error).message).not.toContain(
      "mysql://ci_user:super-secret-db-password@localhost/jiayi",
    );
  });
});
